...
class SettingsRepositoryImpl(
    ...
    override suspend fun saveSetting(periodic: Long, delayed: Long) {
        withContext(dispatcher) {
            ...
            dataStore.edit {
                it[REFRESH_PERIOD_KEY] = periodic
                it[FIRST_LAUNCH_DELAY_KEY] = delayed
            }
            _state.emit(SettingContainer(periodic, delayed))
        }
    }
    ...
    override suspend fun readSetting() {
        withContext(dispatcher) {
            dataStore.data
                .map { prefs ->
                    SettingContainer(
                        periodic = prefs[REFRESH_PERIOD_KEY] ?: SettingContainer.DEFAULT_REFRESH_PERIOD,
                        delayed = prefs[FIRST_LAUNCH_DELAY_KEY] ?: SettingContainer.FIST_LAUNCH_DELAY
                    )
                }
                .collect { settings ->
